    ...
    const PosResOrderReceipt = OrderReceipt => class extends OrderReceipt {
        ...
        onMounted() {
            if(this.env.pos.config.is_custom_receipt){
                const receipt_design_id = this.env.pos.config.design_receipt;
                const order = this._receiptEnv.order;
                const data = {
                    widget: this.env,
                    pos: order.pos,
                    order: order,
                ...
                const xmlDoc = new DOMParser().parseFromString(receipt_design_id, "text/xml");
                ...
                const receipt = qweb.render('receipt_design_id', data);
                document.querySelector('div.pos-receipt').replaceWith(receipt);
            }
        }
    }
